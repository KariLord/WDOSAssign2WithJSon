document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('index.json')
        ...
        .then(data => {
            ...
            localStorage.setItem('indexJsonData', JSON.stringify(data));
        })
        .catch(error => {
...
.then(data => {
    ...
    localStorage.setItem('introHtmlData', JSON.stringify(data));
})
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    .then(response => response.json())
    .then(data => {
        // Fill content in the specified element with id 'depfill'
        document.getElementById('depfill').textContent = data.departmentDescription;

        // Store fetched data in localStorage
        localStorage.setItem('DOWLData', JSON.stringify(data));
    })
    .catch(error => {
        ...
        login() {
            ...
            if (user) {
                if (user.username === 'user') {
                    window.location.href = 'index2.html'; // Redirect to index.html for user
